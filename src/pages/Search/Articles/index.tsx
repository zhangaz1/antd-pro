...
interface ArticlesProps {
  ...
  searchAndArticles: StateType;
  loading: boolean;
}
const Articles: FC<ArticlesProps> = ({
  dispatch,
  searchAndArticles: { list },
  loading,
}) => {
  const [form] = Form.useForm();
  useEffect(() => {
    dispatch({
      type: 'searchAndArticles/fetch',
      payload: {
        count: 5,
      },
    });
  ...
  const fetchMore = () => {
    dispatch({
      type: 'searchAndArticles/appendFetch',
      payload: {
        count: pageSize,
      },
    });
...
export default connect(
  ({
    ...
    searchAndArticles,
    loading: loading.models.searchAndArticles,
  }),
)(Articles);
